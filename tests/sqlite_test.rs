...
#[test]
fn test_sqlite_execution() {
    let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let bin_path = manifest_dir.join("bin").join("sqlite3");
    ...
    loop {
        inst_count += 1;
        assert!(inst_count < 10_000_000, "sqlite3 execution exceeded instruction limit");
        ...
        match Interpreter::step(&mut ctx, &mut mem) {
            Ok(true) => {},
            Ok(false) => break,
            Err(e) => panic!("Interpreter error in sqlite3: {:?}", e),
        }
    ...
    assert_eq!(ctx.exit_code, 0, "sqlite3 exited with non-zero status");
}

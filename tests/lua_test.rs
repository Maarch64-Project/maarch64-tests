...
fn test_lua_execution() {
    ...
    let bin_path = manifest_dir.join("bin").join("lua");
    ...
    loop {
        inst_count += 1;
        assert!(inst_count < 10_000_000, "lua execution exceeded instruction limit");
        ...
        match Interpreter::step(&mut ctx, &mut mem) {
            Ok(true) => {},
            Ok(false) => break,
            Err(e) => panic!("Interpreter error in lua: {:?}", e),
        }
    ...
    assert_eq!(ctx.exit_code, 0, "lua exited with non-zero status");
}
